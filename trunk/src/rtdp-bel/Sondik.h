...
public:
  ...
  int action_;
  double *vec_;
  Vector( const Vector &v ) : dim_(v.dim_), action_(v.action_), vec_(new double[dim_]) { *this = v; }
  Vector( int dim, int action = -1 ) : dim_(dim), action_(action), vec_(new double[dim_]) { }
  virtual ~Vector() { delete[] vec_; }

  const Vector& operator=( const Vector& v ) { action_ = v.action_; for( int i = 0; i < dim_; ++i ) vec_[i] = v.vec_[i]; return(*this); }
  double& operator[]( int i ) { return(vec_[i]); }
  const double& operator[]( int i ) const { return(vec_[i]); }
  const Vector& operator+=( const Vector& v ) { for( int i = 0; i < dim_; ++i ) vec_[i] += v.vec_[i]; return(*this); }
  const Vector& operator*=( double alpha ) { for( int i = 0; i < dim_; ++i ) vec_[i] *= alpha; return(*this); }
  double operator*( const Vector &v ) const
  {
    double sum = 0.0;
    for( int i = 0; i < dim_; ++i ) sum += vec_[i]*v.vec_[i];
    return(sum);
  }
  ...
  {
    double dist = 0.0;
    for( int i = 0; i < dim_; ++i ) dist += fabs(vec_[i]-v.vec_[i]);
    return(dist);
  }
};
...
{
public:
  void clear() { for( size_t i = 0; i < size(); ++i ) delete (*this)[i]; std::vector<const Vector*>::clear(); }
  bool insert( const Vector *alpha, double epsilon )
  {
    for( const_iterator it = begin(); it != end(); ++it ) {
      if( (*it)->l1_dist(*alpha) <= epsilon ) return(false);
    }
    push_back(alpha);
    return(true);
  }
...
{
public:
  void clear() { for( const_iterator it = begin(); it != end(); ++it ) delete *it; std::list<const StandardBelief*>::clear(); }
};
...
public:
  Sondik( const StandardPOMDP &pomdp ) : pomdp_(pomdp), model_(static_cast<const StandardModel&>(*pomdp.model())), numStates_(model_.numStates()), numActions_(model_.numActions()), numObs_(model_.numObs()), best_(new std::pair<double,int>[numObs_]) { }
  virtual ~Sondik() { delete[] best_; } //clear_belief_set(); clear_gamma(); delete[] best_; }
...
static int timestampCmp( const void *p1, const void *p2 )
{
  const Hash<const QBelief,BeliefHash::Data>::Entry** e1 = (const Hash<const QBelief,BeliefHash::Data>::Entry**)p1;
  const Hash<const QBelief,BeliefHash::Data>::Entry** e2 = (const Hash<const QBelief,BeliefHash::Data>::Entry**)p2;
