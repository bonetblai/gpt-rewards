...
inline std::ostream& operator<<( std::ostream& os, const Belief& belief ) { belief.print(os); return(os); }
...
public:
  ...
  struct Data {
    ...
    unsigned updates_;
    unsigned timestamp_;
    Data( double value = 0, bool solved = false, unsigned updates = 0, unsigned timestamp = 0 ) : value_(value), solved_(solved), updates_(updates), timestamp_(timestamp) { }
  };
...
{
  os << "[value=" << data.value_ << ",solved=" << (data.solved_?"true]":"false]") << ",updates=" << data.updates_ << ",timestamp=" << data.timestamp_;
  return(os);
}
