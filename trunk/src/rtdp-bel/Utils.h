...
randomSampling( const std::vector<std::pair<T,double>,Alloc>& v )
{
...
template<class T> T
randomSampling( const std::pair<T,double> *vec, unsigned size )
{
  register double d = drand48();
...
inline int
randomSampling( double *vec, unsigned size )
{
  register double d = drand48();
  for( unsigned i = 0; i < size; ++i ) {
    if( d <= vec[i] ) return(i);
