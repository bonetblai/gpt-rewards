...
public:
  StandardPOMDP( const StandardModel *model = 0, double qlevels = 10, double qbase = 0.95 ) : POMDP(model), quantization_(new Quantization(qlevels,qbase,(model?model->numStates_:0))), qresult_(new QResult(numActions_)), nextobs_(new double[numObs_]) { beliefHash_ = new QBeliefHash; beliefHash_->setQuantization(quantization_); }
