...
template<typename T> inline bool valid_ptr(const T *ptr) {
    return ptr == 0 ? false : (reinterpret_cast<unsigned long>(ptr) & 0x1 ? false : true);
}

template<typename T> inline T* make_valid_ptr(T *ptr) {
    return reinterpret_cast<T*>(reinterpret_cast<unsigned long>(ptr) ^ 0x1);
}

template<typename T> inline T* make_invalid_ptr(T *ptr) {
    return make_valid_ptr(ptr);
}
...
class BeliefCache {
  public:
    struct Entry {
            ...
            if( table_ ) {
                for( int a = 0; a < nactions; ++a ) {
                    for( int o = 0; o < nobs; ++o ) {
                        ...
                        if( valid_ptr(table_[index]) )
                            table_[index] = make_invalid_ptr(table_[index]);
                        //if( table_[index] != 0 && !((unsigned)table_[index]&0x1) )
                        //    table_[index] = (Belief*)((unsigned)table_[index]^0x1);
                    }
	            if( valid_ptr(obs_table_[a]) )
                        obs_table_[a] = make_invalid_ptr(obs_table_[a]);
	            //if( obs_table_[a] && !((unsigned)obs_table_[a]&0x1) )
                    //    obs_table_[a] = (double*)((unsigned)obs_table_[a]^0x1);
                }
            }
        }
            ...
            if( table_ ) {
                for( int a = 0; a < nactions; ++a ) {
                    for( int o = 0; o < nobs; ++o ) {
                        ...
                        if( table_[index] ) {
                            if( !valid_ptr(table_[index]) )
                                table_[index] = make_valid_ptr(table_[index]);
                            delete table_[index];
                        }
                    }
	            if( obs_table_[a] ) {
	                if( !valid_ptr(obs_table_[a]) )
                            obs_table_[a] = make_valid_ptr(obs_table_[a]);
                        delete[] obs_table_[a];
                    }
                }
            }
        ...
        void insertObservations(int action, double *obs, int nactions, int nobs) {
            allocate(nactions);
            if( obs_table_[action] == 0 ) obs_table_[action] = new double[nobs];
            if( !valid_ptr(obs_table_[action]) ) obs_table_[action] = make_valid_ptr(obs_table_[action]);
        ...
        void insertBelief(int action, int obs, const Belief &belief, int nactions, int nobs) {
            ...
            if( table_[index] != 0 ) {
                if( !valid_ptr(table_[index]) ) table_[index] = make_valid_ptr(table_[index]);
                *table_[index] = belief;
            } else {
        ...
        const double* nextPossibleObservations(int action) const {
            if( obs_table_ != 0 ) {
                return valid_ptr(obs_table_[action]) ? obs_table_[action] : 0;
            } else {
                return 0;
            }
        }
        const Belief* belief_ao(int action, int obs, int nactions) const {
            int index = obs * nactions + action;
            if( table_ != 0 ) {
                return valid_ptr(table_[index]) ? table_[index] : 0;
            } else {
                return 0;
            }
        }
        void dump(std::ostream &os, int nactions, int nobs) const {
            ...
            for( int action = 0; action < nactions; ++action ) {
                os << "  table for action=" << action << std::endl
                   << "    nextObs[" << action << "]=[";
                if( valid_ptr(obs_table_[action]) ) {
                    for( int obs = 0; obs < nobs; ++obs )
                        os << " " << obs_table_[action][obs];
                    os << " ]";
	        } else {
                    os << "<null>";
                }
  ...
  public:
    BeliefCache(unsigned n = 11, unsigned m = 5)
      : n_(1<<n), m_(1<<m),
        nmask_((1<<(n+1))-1), lookups_(0), hits_(0) {
        cache_ = new Entry[1<<(n+m)];
    }
    ...
    void print(std::ostream &os) const {
        ...
        for( unsigned i = 0; i < n_; ++i ) {
            for( unsigned j = 0; j < m_; ++j ) {
                os << "Entry(" << i << "," << j << ")=["
                   << "refs=" << cache_[i*m_+j].refs_
                   << ",belief=" << (cache_[i*m_+j].belief_ ? "" : "<null>");
                if( cache_[i*m_+j].belief_ )
                    os << *cache_[i*m_+j].belief_;
                os << std::endl;
            }
        }
    }
...
inline std::ostream& operator<<(std::ostream&os, const BeliefCache &cache) {
    cache.print(os);
    return os;
 }
