...
public:
  StandardBelief( const StandardBelief &belief ) : vec_(0), capacity_(0), size_(0) { *this = belief; }
  StandardBelief( unsigned capacity = 0 ) : vec_(0), capacity_(0), size_(0) { resize(capacity); }
  StandardBelief( const std::vector<std::pair<int,double> > &vector ) : vec_(0), capacity_(0), size_(0) { *this = vector; }
  virtual ~StandardBelief() { if( vec_ ) delete[] vec_; }
  ...
  {
    double sum = 0;
    for( iterator it = begin(); it != end(); ++it ) sum += (*it).second;
    for( iterator it = begin(); it != end(); ++it ) (*it).second /= sum;
  }
  void push_back( int state, double probability ) { resize(size_+1); vec_[size_++] = std::make_pair(state,probability); }
  ...
  virtual Belief::Constructor getConstructor() const { return((Belief::Constructor)&StandardBelief::constructor); }
  virtual int sampleState() const { return(::randomSampling(vec_,size_)); }
  ...
  virtual const Belief& update( const Model *model, int action, int obs ) const
  {
    ...
    assert( sum > 0 );
    for( std::pair<int,double> *ptr = bel_ao_.vec_, *pend = &bel_ao_.vec_[bel_ao_.size_]; ptr != pend; ++ptr ) { ptr->second /= sum; }
    return(bel_ao_);
  }

  virtual Belief* clone() const { return(new StandardBelief(*this)); }
  virtual unsigned hashFunction() const
  {
    ...
    a += (unsigned)vec_[i].first;
    b += ptr[0] + ptr[1];
    if( length == 2 ) { HASH_FINAL(a,b,c); return(c); }
    ++i;
    c += (unsigned)vec_[i].first;
    HASH_MIX(a,b,c);
    ptr = reinterpret_cast<unsigned*>(&vec_[i].second);
    a += ptr[0] + ptr[1];
    if( length == 4 ) { HASH_FINAL(a,b,c); return(c); }
    ++i;
    ptr = reinterpret_cast<unsigned*>(&vec_[i].second);
    b += (unsigned)vec_[i].first;
    c += ptr[0] + ptr[1];
  ...
  virtual const Belief& operator=( const Belief &belief ) { return(operator=(static_cast<const StandardBelief&>(belief))); }
  ...
  virtual bool operator==( const Belief &belief ) const { return(operator==(static_cast<const StandardBelief&>(belief))); }

  // serialization
  static StandardBelief* constructor() { return(new StandardBelief); }
