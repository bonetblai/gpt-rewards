...
StandardPOMDP::learnAlgorithm( Result& result )
{
  ...
  while( (PD.signal_ < 0) && (result.numSteps_ < cutoff_) ) {
    if( PD.verboseLevel_ >= 30 ) {
      *PD.outputFile_ << std::endl
	              << "state=" << state << std::endl
	              << "belief=" << belief << std::endl
		      << "qbelief=" << qbelief << std::endl
		      << "data=" << qdata << std::endl;
    }
    ...
    if( PD.verboseLevel_ >= 30 ) { // print info
      *PD.outputFile_ << "action=" << bestAction << ", obs=" << observation << std::endl;
    }
  }
  ...
  if( PD.useStopRule_ && (result.numSteps_ < cutoff_) ) {
    std::list<BeliefHash::Entry> closed;
    while( !stack.empty() ) {
