...
class History {
  ...
  public:
    ...
    History(unsigned capacity = DEFAULT_CAPACITY)
      : act_seq_(0), num_act_(0), act_cap_(capacity),
        obs_seq_(0), num_obs_(0), obs_cap_(capacity),
        next_(0) {
    ...
    virtual ~History() { delete[] act_seq_; delete[] obs_seq_; }

    // TODO: check border case: num_act = 1, num_obs = 1
    static void initialize(int num_act, int num_obs) {
        assert(num_act > 0);
        assert(num_obs > 0);
