...
QMDPHeuristic::compute()
{
  ...
  do {
      ...
      if( !model_->isAbsorbing(state) ) {
        ...
        for( int action = 0; action < model_->numActions(); ++action ) {
          if( model_->applicable(state,action) ) { // compute value for an action 
            double sum = 0.0;
            const std::vector<std::pair<int,double> > *vec = model_->transition_[state*model_->numActions()+action];
            assert( vec != 0 );
    ...
    if( PD.verboseLevel_ >= 100 ) *PD.outputFile_ << "QMDP_residual=" << error << std::endl;
