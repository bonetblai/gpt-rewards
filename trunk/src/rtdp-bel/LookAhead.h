...
public:
  LookAheadHeuristic( const POMDP *pomdp = 0, const Heuristic *h = 0, int lookahead = 0 ) : pomdp_(pomdp), h_(h), lookahead_(lookahead), nextobs_(new double[pomdp_->numObs()]) { }
  virtual ~LookAheadHeuristic() { delete[] nextobs_; }
  ...
  virtual double value( int state ) const { return(h_->value(state)); }
  virtual double value( const Belief &belief ) const { return(value(lookahead_,belief)); }

  // serialization
  static LookAheadHeuristic* constructor() { return(new LookAheadHeuristic); }
