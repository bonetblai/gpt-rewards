...
public:
  struct Entry {
    ...
    Data data_; 
    Entry *next_;
    Entry( const Key &key, const Data &data, Entry *next ) { key_ = new Key(key); data_ = data; next_ = next; }
    ~Entry() { delete key_; }
  };
...
protected:
  ...
  static const unsigned defaultSize_ = (1<<20);

public:
  Hash( unsigned size = 0 ) : table_(0), size_(0), mask_(0), nentries_(0), freeslots_(0), nlookups_(0), nfound_(0)
  {
    if( size == 0 )
      size_ = defaultSize_;
    else {
  ...
  virtual ~Hash() { clean(); delete[] table_; }
  
  unsigned size() const { return(size_); }
  unsigned nentries() const { return(nentries_); }
  unsigned nlookups() const { return(nlookups_); }
  unsigned nfound() const { return(nfound_); }
  void resetStats() const { nlookups_ = 0; nfound_ = 0; }
  unsigned bucket( Key &key ) const { return(key.hashFunction()&mask_); }

  const Entry* lookup( const Key &key ) const
  {
    unsigned index = bucket(key);
    const Entry *entry = table_[index];
    while( (entry != 0) && !(key == *entry->key_) ) { entry = entry->next_; }
    ++nlookups_;
    nfound_ += (entry?1:0);
    return(entry);
  }
    ...
    for( unsigned i = 0; i < size_; ++i ) {
      if( table_[i] != 0 ) {
        ...
        for( const Entry *e = table_[i]; e != 0; e = e->next_ )
          os << "  Key=" << *e->key_ << ", Data=" << e->data_ << std::endl;
      }
    }
  }
  ...
  struct iterator {
    ...
    Entry **table_;
    unsigned size_;
    iterator( unsigned bucket = 0, Entry *entry = 0, Entry **table = 0, unsigned size = 0 ) : bucket_(bucket), entry_(entry), table_(table), size_(size) { }
    ~iterator() { }
    ...
    void operator++()
    {
      ...
      if( bucket_ < size_ ) {
        entry_ = entry_->next_;
        while( !entry_ && (++bucket_ < size_) ) entry_ = table_[bucket_];
      }
      assert( entry_ || (bucket_ == size_) );
    }
    bool operator!=( const iterator &i ) const { return( (bucket_!=i.bucket_) || (entry_!=i.entry_) ); }
    bool operator==( const iterator &i ) const { return( (bucket_==i.bucket_) && (entry_==i.entry_) ); }
    const iterator& operator=( const iterator &i ) { bucket_ = i.bucket_; entry_ = i.entry_; return(*this); }
  };
  iterator begin()
  {
    unsigned bucket = 0;
    Entry *entry = table_[bucket];
    while( !entry && (++bucket < size_) ) entry = table_[bucket];
  ...
  struct const_iterator {
    ...
    Entry** const table_;
    unsigned size_;
    const_iterator( unsigned bucket = 0, const Entry *entry = 0, Entry** const table = 0, unsigned size = 0 ) : bucket_(bucket), entry_(entry), table_(table), size_(size) { }
    ~const_iterator() { }
    void operator++()
    {
      assert( entry_ || (bucket_ == size_) );
    ...
    bool operator!=( const const_iterator &i ) const { return( (bucket_!=i.bucket_) || (entry_!=i.entry_) ); }
    bool operator==( const const_iterator &i ) const { return( (bucket_==i.bucket_) && (entry_==i.entry_) ); }
    const const_iterator& operator=( const const_iterator &i ) { bucket_ = i.bucket_; entry_ = i.entry_; return(*this); }
  };
